	...
	<main>
		...
		<?php
			if (isset($_GET['viewid'])) {
				$view_sql = "SELECT * FROM inquiries WHERE id = '".$_GET['viewid']."' ";
				$view_res = mysqli_query($link, $view_sql);

				if (mysqli_num_rows($view_res) > 0) {
					while ($result = mysqli_fetch_assoc($view_res)) {
		?>
		<!-- ==================== Mainbar ====================-->
		<div class="main-right" id="mainbar">
			<div class="page-title-box">
				<p class="page-title page-title-box"><?php echo $result['full_name']?></p>
			</div>
			<p class="view-date">Schedule: <?php echo $result['schedule_date']?></p>
			<p class="view-date">Contact: <?php echo $result['contact']?></p>
			<p class="view-date">Email: <?php echo $result['email_address']?></p><br><br>
			<p class="view-date">Remarks:</p>
			<p class="inquiry-remarks"><?php echo nl2br($result['remarks'])?></p>
		</div>
		<!-- ==================== Mainbar end ====================-->
		<?php
					}
