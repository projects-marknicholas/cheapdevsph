...
<body>
	...
	<main>
		...
		<div class="main-right" id="mainbar">
			<p class="page-title">certificates</p>
			<!-- ==================== Attendance table ====================-->
			<div class="attendance-div">
				<div class="table-top">
					<button class="btn-bulkprint">bulk print ID</button>
				...
				<table class="attendance-table" id="table">
					<tr class="attendance-tr">
						<tbody class="attendance-tbody">
							...
							<th class="attendance-th">position</th>
							<th class="attendance-th">schedule</th>
							<th class="attendance-th">member since</th>
					...
					?>
					<tr class="attendance-tr">
						<td class="attendance-td"><?php echo $result['employee_id']?>-00<?php echo $result['id']?></td>
						<td class="attendance-td"><?php echo $result['firstname']?> <?php echo $result['lastname']?></td>
						<td class="attendance-td">
							<?php
								if ($result['position'] == '') {
									echo '<p style="color: red;">no position yet</p>';
								}
								else{
						...
						<td class="attendance-td">
							<?php
								if ($result['time_in'] == '' && $result['time_out_second'] == '') {
									echo '<p style="color: red;">no schedule yet</p>';
								}
								else{
									echo $result['time_in'].'AM-'.$result['time_out_second'].'PM';
								}
							?>
						</td>
						<td class="attendance-td">
							<?php
								if ($result['member_since'] == '') {
									echo '<p style="color: red;">not member yet</p>';
								}
								else{
						...
						<td class="attendance-td">
							<button class="btn-edit" onclick="window.location.href='edit-certificate.php?viewid=<?php echo $result['id']?>';">edit</button>
							<?php
								$btn = "SELECT * FROM certificate WHERE viewid = '".$result['id']."'";
								$btn_query = mysqli_query($link, $btn);

								if (mysqli_num_rows($btn_query) == 0) {
									echo "
										<button class='btn-delete'>not yet edited</button>
									";
								}
								else{
									echo "
										<a href='pdf/?printid=".$result['code']."&setid=".$result['pin']."&uniqid=".$result['id']."' style='text-decoration: none;' class='btn-print' >print certificates</a>
									";
								}
							?>
						</td>
